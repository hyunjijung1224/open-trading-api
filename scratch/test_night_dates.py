...
logger = logging.getLogger("TestNightDates")
...
def test_query(token, base_url, appkey, appsecret, symbol, date_str):
    ...
    try:
        ...
        if output2:
            logger.info(f"[{symbol} | {date_str}] First 3 candles:")
            for i, row in enumerate(output2[:3]):
...
def main():
    ...
    # Let's test different dates for 101W9000
    dates = ["20260615", "20260613", "20260612", "20260611"]
    for date_str in dates:
        test_query(token, base_url, appkey, appsecret, "101W9000", date_str)
        time.sleep(2.0)
